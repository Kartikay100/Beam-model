...
Input data for SFBM verification # 7- Pin-pin overhanging beam with UDL.
'''
...
inputGeom = {'L': 10,   # length of beam, in
        'b': 1, # width of rod, in
...
inputEBC = {'globalNode#': [[0,54], [3,3]], # [node number], [no of boundary values at node]    
...
inputForce = {'globalNode#': [np.array(range(91)).tolist(), np.tile(np.array([1]), 91).tolist()], # [node number], [no of boundary values at node]
            'Values': np.tile([1, 400 ,0], (91,1)).tolist() # [DOF #, value], 0=Tx
        }
