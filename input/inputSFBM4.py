...
Input data for SFBM verification # 4- Pin-pin beam with UDL over a portion of the beam length.
'''
...
inputForce = {'globalNode#': [np.array(range(27,63)).tolist(), np.tile(np.array([1]), 36).tolist()], # [node number], [no of boundary values at node] # UDL from Z=30in to 70 in
            'Values': np.tile([1, 1, 0], (36,1)).tolist() 
            # [DOF #, constant, termZ], 0=Tx
        }
inputMoment = {'globalNode#': [[0], [0]], # [node number], [no of boundary values at node]
            'Values': [[0, 0, 0]] # [DOF #, value0, value1, value 2], 0=Tx, 1=Ty, 3=Rx
        }
